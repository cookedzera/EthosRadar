...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/analyze-activities", async (req, res) => {
    try {
      ...
      if (!userkey || !activities || !Array.isArray(activities)) {
        return res.status(400).json({
          error: "userkey and activities array are required"
        });
      }
      ...
      const reviews = activities.filter((a: any) => a.type === 'review');
      const vouches = activities.filter((a: any) => a.type === 'vouch');

      console.log(`Found ${reviews.length} reviews and ${vouches.length} vouches`);

      if (activities.length === 0) {
        return res.status(400).json({
          error: "No reviews or vouches found for analysis"
        });
      }
      ...
      const formattedActivities = activities.map((activity: any) => {
        ...
        const authorScore = activity.author?.score || 0;
        const content = activity.data?.comment || activity.comment || '';
        const score = activity.data?.score || activity.score || '';
      ...
      const avgAuthorScore = activities.reduce((sum: number, activity: any) =>
        sum + (activity.author?.score || 0), 0) / activities.length;
      ...
      console.log(`Analysis completed for userkey: ${userkey}`);
  ...
  app.get("/api/user-activities/:userkey", async (req, res) => {
    try {
      const { userkey } = req.params;
      ...
      console.log(`Total activities found: ${formattedActivities.length} (${reviews.length} reviews, ${vouches.length} vouches)`);
  ...
  // Complete spider chart analysis endpoint
  app.get("/api/spider-analysis/:userkey", async (req, res) => {
    try {
      const { userkey } = req.params;
      ...
      console.log(`Starting spider chart analysis for userkey: ${userkey}`);

      // Step 1: Fetch user activities
      const activitiesResponse = await fetch(`http://localhost:${process.env.PORT || 5000}/api/user-activities/${encodeURIComponent(userkey)}`);
      ...
      if (!activitiesData.success || !activitiesData.data || activitiesData.data.length === 0) {
        return res.json({
          ...
          data: {
            ...
            avgAuthorScore: 0,
            model: "no-data",
            results: {} // Empty results for users with no activities
          }
        });
      }

      // Step 2: Analyze activities with AI
      const analysisResponse = await fetch(`http://localhost:${process.env.PORT || 5000}/api/analyze-activities`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!analysisResponse.ok) {
        ...
        throw new Error(errorData.error || 'Failed to analyze activities');
      }

      const analysisResult = await analysisResponse.json();
      
      console.log(`Spider chart analysis completed for userkey: ${userkey}`);
      
      res.json(analysisResult);

    } catch (error) {
      console.error("Error in spider chart analysis:", error);
      const errorMessage = error instanceof Error ? error.message : String(error);
      res.status(500).json({
        success: false,
        error: "Failed to complete spider chart analysis",
        details: errorMessage
      });
    }
  });
